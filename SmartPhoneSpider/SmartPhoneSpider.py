...
try:
    ...
    excel.write(sheet_name,0,0,"品牌");
    excel.write(sheet_name,0,1,"手机名");
    excel.write(sheet_name,0,2,"上市时间");
    excel.write(sheet_name,0,3,"屏幕尺寸");
    excel.write(sheet_name,0,4,"屏幕分辨率");
    excel.write(sheet_name,0,5,"操作系统");
    excel.write(sheet_name,0,6,"运行内存");
    excel.write(sheet_name,0,7,"CPU型号");
    ...
    r = requests.get("https:" + smart_phone_url,timeout=10);
    if 200 == r.status_code:
        doc_company = pq(r.text);
        #print(doc_company);
    else:
        print("品牌页http请求失败，错误码：",r.status);
    all_company_a = doc_company("div#openDiv.all-shaix li.pinpai div.pinpai-div-scroll-div a");
    #print("all_company_a:",all_company_a);
    all_company_url = [];
    for item in all_company_a.items():
        a_url_list = [];
        if item.attr("title"):
            a_url_list.append(item.attr("title"));
        else:
            a_url_list.append(item.text());
        a_url_list.append("//product.cnmo.com" + item.attr("href"));
        #print(a_url_list);
        all_company_url.append(a_url_list);
    print(all_company_url);
    i = 1;
    for company in all_company_url:
        company_name = company[0];
        next_page_url = company[1];
        while(next_page_url):
            #print(next_page_url);
    ...
    #############################################################################################################################################################################################################################
            ...
            for item in phone_info_list:
                try:
                    excel.write(sheet_name,i,0,company_name);
                    excel.write(sheet_name,i,1,item["手机名"]);
                    excel.write(sheet_name,i,2,item["上市时间"]);
                    excel.write(sheet_name,i,3,item["屏幕尺寸"]);
                    excel.write(sheet_name,i,4,item["屏幕分辨率"]);
                    excel.write(sheet_name,i,5,item["操作系统"]);
                    excel.write(sheet_name,i,6,item["运行内存"]);
                    excel.write(sheet_name,i,7,item["CPU型号"]);
